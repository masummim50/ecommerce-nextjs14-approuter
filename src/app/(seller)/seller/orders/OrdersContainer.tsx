  ...
  return (
    <div>
      <div className="grid grid-cols-2 gap-2">
        {orders.map((order) => {
          return <Link href={`/seller/orders/${order.id}`} className="bg-white shadow-lg hover:bg-gray-300" key={order.id}>
            <div className="p-2">
            ...
            <div className="p-2 flex justify-between">
                ...
                <div className="">
                  <h2>Total: {order.items.reduce((prev,curr)=> prev+curr.productQuantity,0)} items</h2>
                  <h2>${order.paymentAmount}</h2>
                  </div>
            </div>
            <Divider/>
            <div className="p-2">
                <h2 className="px-3 py-1 bg-green-300 rounded-md inline-block">{order.status}</h2>
            </div>
          </Link>;
        })}
      </div>
