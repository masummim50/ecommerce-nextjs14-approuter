...
const DiscountedPage = async ({searchParams}:{searchParams:{page:string}}) => {
  // fetch popular data
  const data = await fetch(`${baseUrl}/product/discounted?page=${searchParams.page}`, {cache: 'no-store'});
  ...
  <div className="max-w-[1100px]  m-auto text-black dark:text-gray-300">
    {result.data?.length > 0 && (
      <div>
        <h2 className="text-xl font-semibold">Discounted Items</h2>
        <p>
